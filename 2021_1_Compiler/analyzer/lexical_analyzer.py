...
class Syntax:
    ...
    def input(cls, symbol, last_parsed_token):  # input symbol to every DFA.
        result = list(map(lambda spec: spec.input_symbol(
            symbol, last_parsed_token), cls.specs))
        return result
    ...
    while(True):
        ...
        if(symbol == ''):  # end of file
            if(last_syntax_result):
                ...
                print(f"Lexical Analyzer : Accepted")
                print(
                    f"Lexical Analyzer : Token file saved at {output_file_name} successfully")
            else:
                sys.exit(
                    f"Lexical Analyzer : Error-Cannot Parse at line {last_parsed_token_line}")
            break

        syntax_result = list(filter(
            lambda syntax: syntax is not None, Syntax.input(symbol, last_parsed_token)))
